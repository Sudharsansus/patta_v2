...
class OtpService {
  constructor(opts = {}) {
    ...
    this._verified = {}; // verifiedId -> { session, opts, at } — OTP-verified sessions RETAINED
                         // for OTP-free borrows of other parcels (A-Register session reuse).
    ...
    // Cap on RETAINED verified sessions (each pins a browser). Oldest evicted over cap.
    this._maxRetained = parseInt(process.env.MPQR_MAX_RETAINED || '6', 10);
    ...
    this._timer = setInterval(() => {
      this._prunePending(); this._pruneWarm(); this._pruneVerified(); this._ensureWarmPool();
    }, tickMs);
   ...
   */
  async completeVerification(pendingId, otp, opts = {}) {
    const retain = !!opts.retain; // keep the verified session alive for OTP-free borrows
    this._prunePending();
    const pending = this._pending[pendingId];
    ...
    try {
      ...
      if (keepSession && alive) {
        ...
        pending.createdAt = Date.now();
        try { playwrightSession.startKeepAlive(); } catch (_) {}
      } else if (retain && verifiedOk && alive) {
        // Success + retain: keep this verified session so other parcels can be
        // fetched on it with NO new OTP (session reuse). Still refill the warm pool.
        delete this._pending[pendingId];
        this._verified[pendingId] = { session: playwrightSession, opts: pending.opts || {}, at: Date.now(), busy: false };
        this._enforceVerifiedCap();
        try { this._ensureWarmPool(); } catch (_) {}
      } else {
        // Success (no retain) or terminal outcome — free the browser + its launcher
        // slot and refill the warm pool so the next customer still gets a fast send.
        delete this._pending[pendingId];
        try { await playwrightSession.close(); } catch (_) {}
        try { this._ensureWarmPool(); } catch (_) {}
      }
  ...
  /**
   * Borrow: fetch a DIFFERENT parcel's document on an OTP-verified session that was
   * retained by completeVerification(..., { retain: true }) — NO new OTP. Returns
   * { html, pdf } (pdf null if the government rejected the OTP-free borrow).
   */
  async borrowParcel(verifiedId, parcel) {
    const v = this._verified[verifiedId];
    const alive = v && v.session && !v.session._closed && v.session.page && !v.session.page.isClosed();
    if (!alive) throw E.VERIFY_EXPIRED('No retained verified session for this reference (it may have expired)');
    v.at = Date.now(); // touch — extend the retention window on use
    // SERIALIZE borrows on this shared session/page: a second borrow queues behind the
    // first (concurrent goto/fill on one page would corrupt each other / cross parcels).
    const run = async () => {
      const cur = this._verified[verifiedId];
      const stillAlive = cur && cur.session && !cur.session._closed && cur.session.page && !cur.session.page.isClosed();
      if (!stillAlive) throw E.VERIFY_EXPIRED('Retained verified session expired while queued');
      cur.at = Date.now();
      return cur.session.borrowParcelAreg(parcel);
    };
    const prev = v._chain || Promise.resolve();
    const mine = prev.then(run, run); // run after the previous borrow settles (success or fail)
    v._chain = mine.catch(() => {});  // keep the chain alive regardless of this borrow's outcome
    return mine;
  }

  // Keep the retained-session count bounded — each pins a ~250 MB browser. Over the
  // cap, evict the OLDEST (by last use) and close it.
  _enforceVerifiedCap() {
    const ids = Object.keys(this._verified);
    if (ids.length <= this._maxRetained) return;
    ids.sort((a, b) => this._verified[a].at - this._verified[b].at);
    for (const id of ids.slice(0, ids.length - this._maxRetained)) {
      const v = this._verified[id];
      delete this._verified[id];
      if (v && v.session && typeof v.session.close === 'function') Promise.resolve(v.session.close()).catch(() => {});
    }
  }

  _pruneVerified() {
    const now = Date.now();
    const ttlMs = 10 * 60 * 1000; // retained verified sessions live 10 min from last use
    for (const [id, v] of Object.entries(this._verified)) {
      if (now - v.at <= ttlMs) continue;
      delete this._verified[id];
      if (v.session && typeof v.session.close === 'function') Promise.resolve(v.session.close()).catch(() => {});
    }
  }
  ...
  async shutdown() {
    ...
    const backends = [
      ...
      ...Object.values(this._verified || {}).map((v) => v.session),
