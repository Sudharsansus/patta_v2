...
class OtpService {
  ...
  async beginVerification(mobile, opts = {}) {
    ...
    // Send the OTP. If the (usually pre-warmed) browser's govt session was stale — an
    // "INVALID ACCESS" / session-expired / govt hiccup on the ENTRY page, BEFORE any OTP
    // is actually sent — transparently discard it and retry ONCE on a fresh cold browser.
    // This kills the "FIRST user gets 'government portal unavailable'" caused by a stale
    // standing warm tab (its session idled out). Safe: no OTP was sent on the failed
    // attempt, and it only fires for entry/session errors — never a real RATE_LIMITED.
    const RETRY_ENTRY = new Set(['SESSION_EXPIRED', 'GOVT_DOWN']);
    const freshBrowser = async () => {
      metrics.coldLaunch.inc();
      const s = new PlaywrightSession({ headless: this.opts.playwrightHeadless, timeoutMs: this.opts.playwrightTimeoutMs, formKind: this.opts.formKind });
      await s.start();
      return s;
    };
    for (let sendTry = 0; ; sendTry++) {
      try {
        ...
        if (dialogText) {
          ...
          if (sendTry === 0 && RETRY_ENTRY.has(cls.code)) {
            console.warn(`[otp] entry/session failure (${cls.code}) on a ${warmHit ? 'warm' : 'cold'} browser — one fresh-browser retry (no OTP was sent)`);
            try { await playwrightSession.close(); } catch (_) {}
            playwrightSession = await freshBrowser(); warmHit = false;
            continue;
          }
        ...
        if (sendTry === 0 && RETRY_ENTRY.has(cls.code)) {
          console.warn(`[otp] send threw ${cls.code} on the entry page — one fresh-browser retry`);
          try { await playwrightSession.close(); } catch (_) {}
          try { playwrightSession = await freshBrowser(); warmHit = false; continue; }
          catch (e2) { try { await playwrightSession.close(); } catch (_) {} throw error; }
        }
  ...
  _validateParcel(p) {
    ...
    // Undivided land: the government's sub-division dropdown offers a single "-" marker
    // (confirmed live — getSubdivNo returns <subdivcode>-</subdivcode>), and TN issues the
    // patta for it. Accept "-" alongside the normal alphanumeric codes (1, 2A, 5C1, …).
