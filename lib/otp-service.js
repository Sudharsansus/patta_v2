...
class OtpService {
  constructor(opts = {}) {
    ...
    // Proactively recycle a warm browser BEFORE it reaches the token window, so a
    // Send-OTP never grabs a near-expired one (self-healing pool).
    this._warmRelaunchAgeMs = Math.min(this._warmMaxAgeMs - 20000, 2.5 * 60 * 1000);
  ...
  async beginVerification(mobile, opts = {}) {
    ...
    if (!playwrightSession) {
      ...
      playwrightSession = new PlaywrightSession({
        headless: this.opts.playwrightHeadless, timeoutMs: this.opts.playwrightTimeoutMs,
      });
      try {
    ...
    // A parked warm browser's govt session goes STALE (the page expires; keep-alive
    // is off). Re-navigate it FRESH from the govt home page so the OTP is sent on a
    // live session — "go from the home page each time". Cold browsers just navigated
    // in start(), so only warm ones need this. On failure, fall back to a cold launch.
    if (warmHit) {
      try {
