...
async function generateMergedPdf({
  ...
  // Deliver ONLY the government patta + FMB — NO branding / cover page. Customers
  // print these as official land records, so a marketed page is inappropriate.
  // (Set PDF_FRONT_PAGE=on to re-enable a branded cover if ever needed.)
  if (process.env.PDF_FRONT_PAGE === 'on') {
    const frontBytes = await generateFrontPagePdf({
      brandName, brandTagline, district, taluk, village, surveyNo, subdivNo,
    });
