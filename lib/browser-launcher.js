...
// The Chromium sandbox is kept by default (Dockerfile runs as non-root pwuser), which
// works on microVM hosts (Fly). But Fargate-style container runtimes (AWS Lightsail/ECS)
// forbid the user-namespace the sandbox needs, so Chromium won't launch there. Set
// MPQR_NO_SANDBOX=1 in those environments to add --no-sandbox (safe as non-root).
const HARDENED = {
  handleSIGINT: false,
  handleSIGTERM: false,
  handleSIGHUP: false,
  ...
  args: [
    ...
    '--no-first-run',
    '--mute-audio',
    ...(process.env.MPQR_NO_SANDBOX === '1' ? ['--no-sandbox', '--disable-setuid-sandbox'] : []),
  ],
};
