...
class PlaywrightSession extends EventEmitter {
  ...
  async start() {
    ...
    this.context = this.browser.newContext
      ? await this.browser.newContext({ ignoreHTTPSErrors: true, serviceWorkers: 'block' })
  ...
  async _openForm() {
    ...
    await retry(async (bail) => {
      ...
      try {
        ...
        if (/INVALID\s+ACCESS/i.test(await this.page.content().catch(() => ''))) return bail(new Error('INVALID ACCESS on the entry page'));
        throw e;
      }
  ...
  async _fetchParcelViaForm(normalized) {
    await this._navFillSubmitParcel(normalized);
    return this._renderHtmlArtifacts(await this.page.content(), this.page.url());
  }

  /**
   * Navigate the extract form, fill the parcel, echo the verified mobile/OTP, submit,
   * and wait for the result to load — leaving it on this.page. Does NOT render/extract
   * (so it works whether the result is a chitta <table> or an A-Register embed/div).
   * Shared by the chitta borrow (_fetchParcelViaForm) and borrowParcelAreg.
   */
  async _navFillSubmitParcel(normalized) {
    if (!this.page || this.page.isClosed()) throw new Error('Playwright page is closed');
    const t0 = Date.now();
    // Reach the form WITHOUT a new OTP + WITHOUT discarding the verified session.
    // The extract page (aRegisterExtract / chittaExtract) GET-renders the form on the
    // verified cookie. If it doesn't (e.g. it wants an rno), fall back to the access
    // chain — which reliably renders the form (verification may or may not survive; a
    // lost session just yields a rejected borrow, never a wasted OTP).
    let onForm = false;
    try {
      await this.page.goto(this.accessConfig.extractUrl, { waitUntil: 'domcontentloaded', timeout: this.timeoutMs });
      await this.page.waitForSelector('#districtCode', { timeout: Math.min(this.timeoutMs, 10000) });
      onForm = true;
    } catch (_) { onForm = false; }
    if (!onForm) {
      console.log('[borrow] extract page gave no form — falling back to the access chain');
      await this._openForm();
    }
    console.log('[borrow] on form via %s | url=%s', onForm ? 'extract-page' : 'access-chain', this.page.url());
    await this._waitForNetworkIdle(this.page);
    ...
    console.log('[borrow] via-form: result loaded in', Date.now() - t0, 'ms');
  }

  /**
   * BORROW an A-Register for a DIFFERENT parcel on this ALREADY-VERIFIED session,
   * with NO new OTP: re-navigates the areg form and submits, echoing the verified
   * mobile/OTP so the government authorises it. VALIDATES the result is a real
   * A-Register (an expired session yields an INVALID ACCESS / re-rendered form, which
   * must NOT be served as a document). Returns { html, pdf }; pdf is null (rejected)
   * if the government did not return the document. Consumes NO OTP.
   */
  async borrowParcelAreg(parcel) {
    if (!this._verifiedSubmission) throw new Error('No verified session to borrow from');
    if (!this.page || this.page.isClosed()) throw new Error('Playwright page is closed');
    const normalized = normalizeSurvey(parcel, this.formData);
    // A timeout here = the govt re-rendered the form (expired session) → rejected, not
    // a crash. Consumes no OTP, so returning rejected is always safe.
    try {
      await this._navFillSubmitParcel(normalized);
    } catch (e) {
      console.warn('[areg-borrow] submit never reached a result (likely expired session):', e.message);
      return { html: null, pdf: null, rejected: true };
    }
    this.formData = { ...normalized, mobile: this.formData.mobile };

    // Guard against serving a NON-document page (same checks submitOtp uses): an
    // expired session lands on INVALID ACCESS, or the govt re-shows the form, or the
    // parcel has no record. In all of these the borrow is rejected (pdf: null → 422).
    const diag = await this._describePage().catch(() => ({}));
    const stillOnForm = await this.page.$('#districtCode').then(Boolean).catch(() => false);
    const noRecord = /enter\s+valid|no\s+record|not\s+available|no\s+data\s+found/i.test(diag.text || '');
    if (stillOnForm || this._looksExpired(diag.text) || noRecord) {
      console.warn('[areg-borrow] REJECTED — stillOnForm=%s expired=%s noRecord=%s | "%s"',
        stillOnForm, this._looksExpired(diag.text), noRecord, (diag.text || '').slice(0, 140));
      return { html: null, pdf: null, rejected: true };
    }
    ...
    console.log('[areg-borrow] result shape:', JSON.stringify(shape).slice(0, 400));
    let pdf = null;
    try { pdf = await this.captureAregPdf(shape); }
    catch (e) { console.warn('[areg-borrow] capture failed:', e.message); }
    return { html, pdf };
  }
