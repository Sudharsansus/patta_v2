...
class PlaywrightSession extends EventEmitter {
  ...
  async start() {
    ...
    try {
      ...
      console.log('[otp] TNSERVICES Patta form ready (#districtCode present)');
      return this;
    } catch (error) {
      await this.close();
      throw new Error(`Unable to open the TNSERVICES Patta form: ${error.message}`);
    }
  }
  ...
  async _openForm() {
    ...
    await retry(async (bail) => {
      ...
      const entry = this.page.getByText('View Patta / Chitta / FMB')
        .or(this.page.locator('a[href*="chittaNewRuralTamil"]'))
        .or(this.page.getByRole('link', { name: /patta.*chitta/i }))
        .first();
      try {
  ...
  async fetchWithCookies(cookies, survey, mobile, otp) {
    ...
    await this.page.goto('https://eservices.tn.gov.in/eservicesnew/land/chittaExtract_en.html?lan=en', {
  ...
  startKeepAlive(intervalMs) {
    ...
    const ms = Math.max(5000, Number(intervalMs || process.env.MPQR_KEEPALIVE_MS || 30000));
  ...
  async _fetchParcelViaForm(normalized) {
    if (!this.page || this.page.isClosed()) throw new Error('Playwright page is closed');
    const t0 = Date.now();
    const url = 'https://eservices.tn.gov.in/eservicesnew/land/chittaExtract_en.html?lan=en';
