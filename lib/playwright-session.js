...
class PlaywrightSession extends EventEmitter {
   ...
   * with NO new OTP. Drives the areg form via _fetchParcelViaForm (which echoes the
   * verified mobile/OTP so the government authorises the submit), then captures the
   * government's own rendered A-Register page. Returns { html, pdf } — pdf is null if
   * the government rejected the borrow (e.g. it binds the session to the OTP'd parcel).
   * Consumes NO OTP, so a failure never wastes one.
   */
  async borrowParcelAreg(parcel) {
    ...
    await this._fetchParcelViaForm(normalized);   // nav + fill + submit (no OTP); leaves result on the page
    this.formData = { ...normalized, mobile: this.formData.mobile };
